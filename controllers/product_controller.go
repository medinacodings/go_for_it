...
func (c *ProductController) CreateProduct(ctx *fiber.Ctx) {
	var product models.Product
	if err := ctx.BodyParser(&product); err != nil {
		ctx.Status(400).JSON(fiber.Map{"error": "Invalid request"})
		return
	}
...
func (c *ProductController) GetAllProducts(ctx *fiber.Ctx) {
...
func (c *ProductController) GetProductByID(ctx *fiber.Ctx) {
	id := ctx.Params("id")
	product, err := c.productService.GetProductByID(id)
	if err != nil {
		ctx.Status(404).JSON(fiber.Map{"error": "Product not found"})
	}
	ctx.JSON(product)
}
